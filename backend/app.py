...
def oauth_callback():
    try:
        ...
        state['youtube_connected'] = True
        save_state(state)
        return """<html><body style="background:#0a0a0f;color:#00e676;font-family:sans-serif;text-align:center;padding:60px">
        <h2>✅ YouTube Connected!</h2><p style="color:#f0f0f5">সফলভাবে connect হয়েছে! এই window বন্ধ করুন।</p>
        <script>setTimeout(()=>window.close(),2000)</script></body></html>"""
    except Exception as e:
        return f"""<html><body style="background:#0a0a0f;color:#ff3d5a;font-family:sans-serif;text-align:center;padding:60px">
        <h2>❌ Error</h2><p style="color:#f0f0f5">{str(e)}</p></body></html>"""
